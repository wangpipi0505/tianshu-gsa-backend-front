...
/** 路由 → 当前页面展示的指令模板分组（全量声明：仅展示命中的分组，按声明顺序排列） */
...
export const PAGE_GREETINGS: Record<string, string> = {
  ...
  '/analytics':
    '您好！当前处于**统计分析工作台**，研判助手已接入实时态势事实层与统计分析引擎。\n\n支持通过自然语言进行**统计口径切换、时空聚合分析、事件影响对比、研判成果沉淀与分析模板复用**，也可以发起历史切片复盘与场景构建。输入研判指令或点击上方模板即可开始。',
  '/ontology':
    '您好！当前处于**领域本体工作台**，研判助手已接入领域本体模型与知识图谱检索引擎。\n\n支持通过自然语言进行**本体概念检索、领域本体定义查询、实体关联关系规则查询与图谱浏览**，也可以发起态势场景构建。输入研判指令或点击上方模板即可开始。'
}

/** 路由 → 未识别指令时的页面化能力清单（按当前业务口径引导改写） */
export const PAGE_FALLBACK_HINTS: Record<string, string[]> = {
  '/workbench': [
  ...
  '/fusion': [
    '**融合任务与数据源**：如"查看当前融合任务的处理进度"、"注册一个新的雷达观测数据源"',
    '**候选关联确认**：如"查看候选关联确认列表并确认融合结果"',
    '**版本与产品发布**：如"发布新资产版本，生成数据产品包装"',
    '**场景构建**：如"在海峡东侧构建一个我方空中巡逻阵位目标"'
  ],
  '/analytics': [
    '**统计口径与聚合**：如"切换到\'仅真实态势\'口径进行统计分析"',
    '**事件影响对比**：如"对重点目标发起事件前后影响对比分析"',
    '**成果沉淀与模板**：如"将当前分析结果沉淀为专题研判成果"',
    '**历史切片复盘**：如"复盘 VIPER-01 战机的历史航迹"'
  ],
  '/ontology': [
    '**本体概念检索**：如"在知识图谱中搜索目标本体相关概念"',
    '**本体定义查询**：如"查看南海方向态势场景的领域本体定义"',
    '**场景构建**：如"构建我国南海与菲律宾方向的态势场景"'
  ]
}
...
点击确认后，系统将执行：**生成资产版本号 → 固化数据产品包装（专题包 + 事实层快照 + 版本变更日志）→ 发布并写入版本历史**。请在融合工作台右侧【资产发布】面板完成最终确认，发布记录可在版本历史中回溯与对比。`,
  ...
  // 场景：统计分析与研判（分析工作台页面口径）
  scenario_analytics_stats: [
    {
      id: 'MSG-USER-ANALYTICS',
      sender: 'user',
      content: '切换到"仅真实态势"口径进行统计分析。',
      timestamp: ''
    },
    {
...
如需将本次口径与分析配置固化为可复用模板，或将当前结果沉淀为专题研判成果，可继续下达相应指令。`,
  ...
  // 场景：领域本体与知识检索（本体工作台页面口径）
  scenario_ontology_query: [
    {
      id: 'MSG-USER-ONTOLOGY',
      sender: 'user',
      content: '在知识图谱中搜索目标本体相关概念。',
      timestamp: ''
    },
    {
      id: 'MSG-AGENT-ONTOLOGY',
      sender: 'agent',
      content: `已完成"目标"本体的知识图谱检索，命中核心概念与关联结构如下：

- **核心实体类**：目标（Target）下辖海上 / 空中 / 陆基 / 水下四个子类，每一子类挂接型号、归属、机动能力等特征属性；
- **关联关系**：目标—事件（参与 / 引发）、目标—目标（伴飞 / 结队 / 打击 / 预警指挥）、目标—区域（活动于 / 管控于）三类关系已在图谱中连通；
- **事实挂接**：当前场景中的重点实体（护航编队、突防战机、南海对峙目标）均已按本体模型实例化，可从概念节点下钻到具体目标实例。

可在本体工作台的图谱画布中展开上述概念节点查看实例分布，检索结果已同步高亮相关关系边。`,
      timestamp: '',
      intentUnderstanding: {
        rawPrompt: '在知识图谱中搜索目标本体相关概念。',
        intentCategory: 'ask_knowledge',
        intentTitle: '领域本体概念检索与知识图谱关联下钻',
        targetScope: ['目标（Target）本体概念'],
        spatialScope: '不涉及空间范围',
        timeScope: '当前本体版本',
        actionSequence: ['解析检索概念', '匹配本体模型节点', '高亮图谱关联结构'],
        confidence: 0.94,
        isConfirmed: true
      }
    }
