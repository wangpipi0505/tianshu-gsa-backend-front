<template>
  ...
  >
    <div v-if="target" class="inspector-container">
      <!-- 目标顶部摘要卡片 -->
      <div class="target-summary-card tactical-panel">
        ...
        <div class="summary-grid">
          ...
          <div class="grid-item">
            ...
            <span class="v text-cyan">{{ target.confidence ? (target.confidence * 100).toFixed(0) : '96' }}%</span>
          </div>
          <div class="grid-item">
          ...
          <div class="grid-item">
            ...
            <span class="v">{{ target.speedKnots }} 节 ({{ formatSpeed(target.speedKnots)?.mach || '巡航' }}) / {{ target.headingDeg }}°</span>
          </div>
        </div>
        ...
        <div class="summary-actions">
          ...
          <el-button size="small" plain @click="emit('ask-agent', `深度研判目标 ${target.codeName} 的目标特性、活动规律与威胁程度`)">
      ...
      <el-tabs v-model="activeTab" type="card" class="inspector-tabs">
        ...
        <el-tab-pane label="基本态势" name="basic">
          <div class="tab-scroll-box">
            ...
            <div class="detail-row">
              ...
              <span class="val">{{ target.firstSeenTime || '2026-08-25 13:00:00' }}</span>
            </div>
            <div class="detail-row">
              <span class="label">最后更新时间:</span>
              <span class="val">{{ target.lastSeenTime || '2026-08-25 15:20:00' }}</span>
            </div>
            <div class="detail-row">
              <span class="label">观测数据集来源:</span>
              <span class="val text-cyan">{{ target.sourceDatasets ? target.sourceDatasets.join(', ') : '多源时空融合中台 (v2.1)' }}</span>
            </div>
        ...
        <el-tab-pane label="目标高维特性" name="characteristic">
          <div class="tab-scroll-box">
            <!-- 光学特性 -->
            <div v-if="target.opticalFeatures || target.opticalCharacteristic" class="char-block">
              ...
              <div class="char-grid">
                <div class="c-item">
                  ...
                  <span class="v" v-else-if="target.opticalCharacteristic">长 {{ target.opticalCharacteristic.shapeFeature?.lengthMeters }} 米 × 翼展 {{ target.opticalCharacteristic.shapeFeature?.widthMeters }} 米</span>
                </div>
                <div class="c-item">
                ...
                <div class="c-item">
                  ...
                  <span class="v text-amber">{{ target.opticalFeatures?.infraredHotspotCount || 2 }} 处主发动机喷口热斑</span>
                </div>
                <div class="c-item">
                  <span class="k">低可探测涂层:</span>
                  <span class="v text-cyan">{{ target.opticalFeatures?.stealthCoatingDetected ? '检测到吸波复合涂层' : '常规金属涂装' }}</span>
                </div>
                <div class="c-item full">
                  <span class="k">光学识别置信度:</span>
                  <span class="v text-green font-mono">{{ ((target.opticalFeatures?.confidence || 0.96) * 100).toFixed(0) }}% (高精度几何匹配)</span>
                </div>
              </div>
            </div>

            <!-- 雷达特性 -->
            <div v-if="target.radarFeatures || target.radarCharacteristic" class="char-block" style="margin-top: 14px">
              ...
              <div class="char-grid">
                <div class="c-item">
                  ...
                  <span class="v text-cyan font-mono">{{ target.radarFeatures?.rcsMeanSqMeters || 2.8 }} ㎡ (RCS)</span>
                </div>
                <div class="c-item">
                  <span class="k">多普勒频移:</span>
                  <span class="v font-mono">{{ target.radarFeatures?.dopplerShiftHz || 1250 }} Hz</span>
                </div>
                <div class="c-item full">
                  <span class="k">探测工作频段:</span>
                  <span class="v text-cyan">{{ target.radarFeatures?.frequencyBand || 'X波段有源相控阵' }}</span>
                </div>
                <div class="c-item">
                  <span class="k">脉冲宽度/重频:</span>
                  <span class="v font-mono">{{ target.radarFeatures?.pulseWidthUs || 12.5 }} μs / {{ target.radarFeatures?.priUs || 85 }} μs</span>
                </div>
                <div class="c-item">
                  <span class="k">信号调制类型:</span>
                  <span class="v text-cyan">{{ target.radarFeatures?.modulationType || '线性调频脉冲压缩' }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="tab-scroll-box">
            ...
            <div v-for="evi in evidenceList" :key="evi.id" class="evidence-card tactical-panel">
              <div class="evi-header">
                ...
                <el-tag size="small" type="success">{{ ((evi.confidenceScore || 0.95) * 100).toFixed(0) }}% 置信度</el-tag>
              </div>
        ...
        <!-- 选项卡 4: 历史相似度研判 -->
        <el-tab-pane label="历史相似目标" name="historical">
          <div class="tab-scroll-box">
            <div class="section-title">基于智能知识库的目标特性余弦相似度检索匹配</div>
            <div v-for="item in HISTORICAL_SIMILAR_TARGETS" :key="item.targetCode" class="hist-card">
              <div class="hist-header">
                ...
                <span class="hist-score">{{ (item.similarityScore * 100).toFixed(1) }}% 相似度</span>
              </div>
